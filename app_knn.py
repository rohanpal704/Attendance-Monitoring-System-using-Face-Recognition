...
def train_model():
    faces = []
    labels = []
    userlist = os.listdir('datasets')
    for user in userlist:
        for imgname in os.listdir(f'datasets/{user}'):
            img = cv2.imread(f'datasets/{user}/{imgname}')
    ...
    joblib.dump(knn,'static/face_recognition_model.h5')
...
def add():
    ...
    userimagefolder = 'datasets/'+newusername+'_'+str(newuserid)
